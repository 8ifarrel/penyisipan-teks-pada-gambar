...
from app.utils.png_io import InvalidPngError, decode_png, remember_source_compression_profile


def open_uploaded_image(file_storage):
  """
  Membuka file upload sebagai RgbImage tanpa melempar exception ke
  caller. Mengembalikan None jika berkas tidak ada/kosong atau bukan
  citra yang bisa dibaca sama sekali (rusak/bukan PNG RGB 24-bit yang
  didukung), sehingga route bisa menampilkan pesan error yang ramah
  alih-alih crash.

  Bytes mentahnya dibaca lebih dulu supaya ukuran kompresi PNG aslinya
  bisa direkam lewat remember_source_compression_profile(), dipakai
  save_png() nanti agar level kompresi citra stego mengikuti citra cover
  aslinya.
  """
